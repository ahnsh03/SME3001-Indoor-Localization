"""
V16 — V15와 동일 무결성·데이터 경로에, **Train CV Optuna 목적함수**만 `composite|c|b|bc` 로 선택 가능하다.

환경 변수(우선순위): ``V16_FUSION_OBJECTIVE`` → ``V15_FUSION_OBJECTIVE`` → 기본 ``composite``.

- ``composite``: V15 단일 단계와 동일 — ``w·RMSE(Wi‑Fi‑Guide)+(1-w)·RMSE(Fusion‑ReWeight)``.
- ``c``: Train CV **Fusion‑ReWeight(Step C)** 만 최소화(v13_fix 단일 목적과 동일 취지).
- ``b``: Train CV **Fusion‑HardGate(Step B)** 만 최소화.
- ``bc``: ``V16_BC_WEIGHT_B·RMSE(B)+(1-V16_BC_WEIGHT_B)·RMSE(C)`` (기본 가중 B=0.5).

2단계(``V15_TWO_PHASE``) 2차 Optuna: ``composite`` 는 V15와 같이 **Step C RMSE만** 사용한다.
...
try:
    ...
    raise ImportError("V16에는 optuna가 필요합니다: py -3 -m pip install optuna") from e
...
from indoor_fusion_pipeline_v13_fix import (
    ...
    mean_cv_rmse_step_b_from_assembled,
...
def resolve_v16_fusion_objective() -> Tuple[str, float]:
    """``V16_FUSION_OBJECTIVE`` 또는 ``V15_FUSION_OBJECTIVE`` → ``composite``|``c``|``b``|``bc`` 및 BC 가중."""
    raw = (os.environ.get("V16_FUSION_OBJECTIVE") or os.environ.get("V15_FUSION_OBJECTIVE") or "composite").strip().lower()
    raw = raw.replace("-", "_")
    aliases = {
        "reweight": "c",
        "step_c": "c",
    ...
    if mode not in ("composite", "c", "b", "bc"):
        raise ValueError(
            f"Invalid fusion objective {raw!r}: use composite | c | b | bc "
            "(set V16_FUSION_OBJECTIVE or V15_FUSION_OBJECTIVE)."
        )
    wb = float(os.environ.get("V16_BC_WEIGHT_B", "0.5"))
    wb = float(min(0.95, max(0.05, wb)))
    return mode, wb


V16_FUSION_OBJECTIVE_MODE, V16_BC_WEIGHT_B = resolve_v16_fusion_objective()
...
def make_objective_v16_primary(
    fold_np: List[Dict[str, Any]],
    lg: LocGeomPack,
    n_rank: int,
    n_prof: int,
    ...
    """단일 단계 Optuna: ``fusion_mode`` 에 따라 Train CV 목적값을 반환한다."""
    nr = max(1, int(n_rank))
    npv = max(1, int(n_prof))
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        if mode == "composite":
            ...
            rm_a = float("nan")
        rm_b = mean_cv_rmse_step_b_from_assembled(fold_np, lg, **kw)
        ...
        if mode == "composite":
            ...
            loss = wb * float(rm_b) + (1.0 - wb) * float(rm_c)

        trial.set_user_attr("cv_kw", kw)
        if mode == "composite" and np.isfinite(rm_a):
            trial.set_user_attr("mean_cv_rmse_wifi_guide_m", float(rm_a))
        trial.set_user_attr("mean_cv_rmse_fusion_hardgate_m", float(rm_b))
        trial.set_user_attr("mean_cv_rmse_fusion_reweight_m", float(rm_c))
        trial.set_user_attr("optuna_objective_value", float(loss))
        if mode == "composite":
...
def make_objective_v16_step_fusion_fixed_blend(
    fold_np: List[Dict[str, Any]],
    lg: LocGeomPack,
    gate_lo: float,
    gate_hi: float,
    ...
    """2단계 2차: 가이드 고정 후 Step B / C / BC / composite(=C) 만 탐색."""
    glo = float(min(gate_lo, gate_hi))
    ghi = float(max(gate_lo, gate_hi))
    ...
    mode = str(fusion_mode).strip().lower()
    wb = float(bc_weight_b)
    phase2_mode = "c" if mode == "composite" else mode
...
def save_plots_v15(
    ...
    triples = [
        (xy_a[:, 0], xy_a[:, 1], f"{STAGE_WIFI_GUIDE} (V16 블렌드+KNN)", rmse_a),
    ...
    fig.suptitle("V16 — Train-only priors / 검증 단일 Forward", fontsize=12, fontweight="bold")
...
def save_cdf_v15(out_dir: Path, ea: np.ndarray, eb: np.ndarray, ec: np.ndarray, ra: float, rb: float, rc: float) -> None:
    ...
    plt.ylabel("CDF")
    plt.title("V16 CDF (validation, single forward)")
...
def main() -> int:
    ...
    fusion_mode = V16_FUSION_OBJECTIVE_MODE
    bc_wb = V16_BC_WEIGHT_B
    print(
        f"\n[V16] 학습 {len(train_df)}행 | 검증 {len(val_df)}행 | K-Fold={n_splits} | fusion_objective={fusion_mode}"
        + (f", bc_weight_b={bc_wb:.3f}" if fusion_mode == "bc" else ""),
        flush=True,
    )
    ...
    print(
        f"[V16] Train-OOF τ 후보(raw→적용): [{gate_lo_raw:.2f},{gate_hi_raw:.2f}]→[{gate_lo:.2f},{gate_hi:.2f}] m | meta={gate_meta}",
        flush=True,
    )
    print(
        f"[V16] 탐색 프로필={tuning['name']}: inflate∈[{tuning['inflate_lo']},{tuning['inflate_hi']}], "
    ...
    print(f"[V16] Train CV 상위 V3 프로필 {len(v3_profiles)}개: {v3_profiles[:5]}…", flush=True)
    ...
    print(f"[V16] 캘리브 캐시 {len(calib_ranks)}개 (train 순수 {STAGE_WIFI_GUIDE} CV)", flush=True)
    ...
    phase2_desc = (
        "2차: Train CV RMSE(Fusion-ReWeight)만 (fusion_objective=composite와 동일)"
        if fusion_mode == "composite"
        else (
            f"2차: Train CV {bc_wb:.3f}·RMSE(B)+(1-{bc_wb:.3f})·RMSE(C)"
            if fusion_mode == "bc"
            else (
                "2차: Train CV RMSE(Fusion-HardGate)만"
                if fusion_mode == "b"
                else "2차: Train CV RMSE(Fusion-ReWeight)만"
            )
        )
    )
    if V15_TWO_PHASE:
        print(
            f"[V16] 2단계: (1) Train CV {STAGE_WIFI_GUIDE} 최소화 {N_OPTUNA_TRIALS_STEPA} trials "
            f"(2) 가이드 고정 후 {phase2_desc}, {N_OPTUNA_TRIALS} trials",
            flush=True,
        )
        study_a = optuna.create_study(direction="minimize", sampler=sampler_a, study_name="v16_wifi_guide")
        study_a.optimize(
        ...
        study_b = optuna.create_study(direction="minimize", sampler=sampler_b, study_name="v16_fusion_fixed_guide")
        study_b.optimize(
            make_objective_v16_step_fusion_fixed_blend(
                fold_np, lg, gate_lo, gate_hi, fixed_guide, tuning, fusion_mode, bc_wb
            ),
        ...
        for tr in study_b.trials:
            ...
            ua = tr.user_attrs or {}
            row: Dict[str, Any] = {
                "phase": "fusion_tuning_phase2",
                "fusion_objective": fusion_mode,
                "trial": tr.number,
                "optuna_objective_value": float(tr.value),
                **tr.params,
                **{f"fixed_{k}": v for k, v in fixed_guide.items()},
            }
            if "mean_cv_rmse_fusion_hardgate_m" in ua:
                row["mean_cv_rmse_fusion_hardgate_m"] = float(ua["mean_cv_rmse_fusion_hardgate_m"])
        ...
        if fusion_mode == "composite":
            print(
                f"[V16] 단일 단계: w·TrainCV_RMSE({STAGE_WIFI_GUIDE})+(1-w)·TrainCV_RMSE({STAGE_FUSION_REWEIGHT}), "
                f"w={V15_STEPA_LOSS_WEIGHT:.3f}",
                flush=True,
            )
        elif fusion_mode == "c":
            print(f"[V16] 단일 단계: Train CV {STAGE_FUSION_REWEIGHT} RMSE만 최소화", flush=True)
        elif fusion_mode == "b":
            print(f"[V16] 단일 단계: Train CV {STAGE_FUSION_HARDGATE} RMSE만 최소화", flush=True)
        else:
            print(
                f"[V16] 단일 단계: Train CV {bc_wb:.3f}·RMSE({STAGE_FUSION_HARDGATE})+"
                f"{1.0 - bc_wb:.3f}·RMSE({STAGE_FUSION_REWEIGHT}) 최소화",
                flush=True,
            )
        obj = make_objective_v16_primary(
            fold_np,
            lg,
            len(calib_ranks),
            len(v3_profiles),
        ...
        hf_fus = float(bt.params["huber_f_fusion"])
        ith = float(bt.params["irls_residual_thresh_m"])
        ua_bt = bt.user_attrs or {}
        best_cv_a = float(ua_bt["mean_cv_rmse_wifi_guide_m"]) if "mean_cv_rmse_wifi_guide_m" in ua_bt else None
        for tr in study.trials:
            if tr.state != optuna.trial.TrialState.COMPLETE:
                continue
            ua = tr.user_attrs or {}
            row: Dict[str, Any] = {
                "phase": f"v16_primary_{fusion_mode}",
                "fusion_objective": fusion_mode,
                "trial": int(tr.number),
                "optuna_objective_value": float(tr.value),
            }
            if fusion_mode == "composite":
                row["composite_loss"] = float(tr.value)
    ...
    ua_final = bt.user_attrs or {}
    rm_b_tr = float(ua_final.get("mean_cv_rmse_fusion_hardgate_m", float("nan")))
    rm_c_tr = float(ua_final.get("mean_cv_rmse_fusion_reweight_m", float("nan")))

    print("\n" + "=" * 72)
    print("V16 — Train-only priors + Train CV Optuna (검증 1회)")
    print("=" * 72)
    if best_cv_a_final is not None:
        print(f"  [Train CV] {STAGE_WIFI_GUIDE} (가이드 최적화 단계) mean RMSE = {best_cv_a_final:.4f} m", flush=True)
    print(
        f"  [Train CV] Optuna minimized = {float(bt.value):.4f} (fusion_objective={fusion_mode}"
        + (f", bc_weight_b={bc_wb:.3f}" if fusion_mode == "bc" else "")
        + ")",
        flush=True,
    )
    if np.isfinite(rm_b_tr):
        print(f"  [Train CV] {STAGE_FUSION_HARDGATE} mean RMSE (best trial) = {rm_b_tr:.4f} m", flush=True)
    if np.isfinite(rm_c_tr):
        print(f"  [Train CV] {STAGE_FUSION_REWEIGHT} mean RMSE (best trial) = {rm_c_tr:.4f} m", flush=True)
    print(
    ...
    single_integrity = {
        ...
        "c": f"Single Optuna: minimize TrainCV_RMSE({STAGE_FUSION_REWEIGHT}) only. ",
        "b": f"Single Optuna: minimize TrainCV_RMSE({STAGE_FUSION_HARDGATE}) only. ",
        "bc": (
            f"Single Optuna: minimize wb·TrainCV_RMSE({STAGE_FUSION_HARDGATE})+(1-wb)·TrainCV_RMSE({STAGE_FUSION_REWEIGHT}) "
            f"(wb={bc_wb}). "
        ),
    }[fusion_mode]
    two_integrity = (
        f"Two-phase Optuna: {STAGE_WIFI_GUIDE} then phase-2 fusion objective={fusion_mode} (train CV). "
        + (f"Phase-2 uses wb={bc_wb} for bc. " if fusion_mode == "bc" else "")
    )
    summ = {
        "pipeline": "V16",
        "fusion_objective": fusion_mode,
        "fusion_bc_weight_b": float(bc_wb) if fusion_mode == "bc" else None,
        "integrity": (
            ...
            + (two_integrity if V15_TWO_PHASE else single_integrity)
        ...
        "train_cv_best_optuna_objective": float(bt.value),
        "train_cv_best_fusion_hardgate_rmse_m": float(rm_b_tr) if np.isfinite(rm_b_tr) else None,
        "train_cv_best_fusion_reweight_rmse_m": float(rm_c_tr) if np.isfinite(rm_c_tr) else None,
    ...
    if save_plots:
        ...
        print("[V16] --no-plots: PNG 생략", flush=True)
...
if __name__ == "__main__":
    ...
    cli_entrypoint(Path(__file__), main, output_artifact_include_prefixes=("v16_",))
