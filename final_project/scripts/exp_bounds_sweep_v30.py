...
def main() -> None:
    ...
    clip_after_affine = [True, False]
    for m in margins:
        xb = (X0_BOUNDS[0] - m, X0_BOUNDS[1] + m)
        yb = (Y0_BOUNDS[0] - m, Y0_BOUNDS[1] + m)
        for do_clip in clip_after_affine:
            ...
            for u in range(700):
                ...
                # clip if enabled (affine can move outside optimization bounds)
                if do_clip:
                    preds[0, u] = np.clip(float(xy[0]), xb[0], xb[1])
                    preds[1, u] = np.clip(float(xy[1]), yb[0], yb[1])
                else:
                    preds[0, u] = float(xy[0])
                    preds[1, u] = float(xy[1])
            rmse = _rmse(p[:, :700], preds)
            print(f"bounds_margin={m:.1f} clip_after_affine={do_clip}: rmse={rmse:.3f}")
